/**
 * OUTPUT · Despacho Abogados Demo · Recuperación de leads
 * Simulación de recuperación de oportunidades abandonadas.
 * Sin fetch. Sin secretos. Datos 100% ficticios.
 */
import { useState } from 'react';
import { MOCK_LEADS_ABANDONO_ABOGADOS } from '../../verticals/abogados/mockData.js';
...
export function AbogadosRecovery() {
  const [leads, setLeads] = useState(MOCK_LEADS_ABANDONO_ABOGADOS);
  ...
  return (
    <div style={{ padding: 20 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: '#6b7280', marginTop: 2 }}>
                  {AREA_EMOJI[l.area] ?? '⚖️'} {l.asunto_interes} · {l.dias_inactivo}d sin actividad
                </div>
      ...
      <p style={{ marginTop: 16, fontSize: 11, color: '#9ca3af', textAlign: 'center' }}>
        ⚠️ Simulación demo · Las acciones no envían emails ni mensajes reales · Datos ficticios · Sin contacto externo
      </p>
    </div>
  );
}
