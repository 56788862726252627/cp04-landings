/**
 * OUTPUT · Clínica Dental Demo · Pantalla 3: Recuperación de leads
 * Visualiza secuencias de recuperación simuladas por pasos.
 * No envía nada. No llama a servicios externos. Solo demo interna.
 */
import { useState } from 'react';
import { Card, Badge, FicticioLabel } from '../../core/AppShell.jsx';
import { MOCK_LEADS_ABANDONO } from '../../verticals/dental/mockData.js';
    ...
    <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{
          ...
          fontSize: 14, fontWeight: 700, color: step.estado === 'simulado' ? '#15803d' : '#374151',
          flexShrink: 0,
        }}>
    ...
    <Card padding="0" style={{ marginBottom: 16 }}>
      ...
      >
        <div style={{ flex: 1 }}>
          ...
          <div style={{ marginTop: 6, fontSize: 13, color: '#6b7280' }}>
            🦷 {lead.tratamiento} · Abandono en: {lead.paso_abandono}
          </div>
      ...
      {expanded && (
        <div style={{ borderTop: '1px solid #f3f4f6', padding: '16px 20px' }}>
          ...
          <div style={{ padding: '12px 14px', background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 8, marginBottom: 12, fontSize: 13, color: '#92400e' }}>
...
export function DentalRecovery() {
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ fontSize: 20, fontWeight: 800, color: '#111827', margin: '0 0 4px' }}>🔄 Recuperación de leads</h2>
        <p style={{ margin: 0, fontSize: 14, color: '#6b7280' }}>Secuencias de recuperación ante abandono · Sin envíos reales · Datos ficticios</p>
      </div>

      {/* Stats rápidas */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: 20 }}>
        {[
          { label: 'Abandonos detectados', value: MOCK_LEADS_ABANDONO.length, icon: '⚠️' },
          { label: 'Recuperados', value: MOCK_LEADS_ABANDONO.filter(l => l.estado_recuperacion === 'recuperado').length, icon: '✅', color: '#059669' },
          { label: 'Tasa recuperación', value: '50%', icon: '📈', color: '#2563eb', ficticio: true },
          { label: 'Valor recuperado est.', value: '2.800 €', icon: '💰', color: '#d97706', ficticio: true },
        ].map(s => (
      ...
      {/* Lista de leads */}
      {MOCK_LEADS_ABANDONO.map(lead => (
        <LeadCard key={lead.id} lead={lead} />
      ))}

      {/* Caso de prueba: abandono antes de reservar */}
      <Card title="Caso de prueba: Abandono antes de reservar" padding="16px" style={{ background: '#f0f9ff', border: '1px solid #bae6fd' }}>
        <p style={{ margin: '0 0 12px', fontSize: 13, color: '#0369a1', lineHeight: 1.6 }}>
          Este es el caso de prueba obligatorio <strong>#4 — Abandono sin reservar</strong>.<br />
          El sistema detecta que un usuario inició el flujo de chatbot pero no completó la reserva de cita.
          Se activa automáticamente una secuencia de recuperación escalonada (inmediato → 24h → 72h).
          <strong> Ningún paso envía mensajes reales.</strong>
        </p>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', fontSize: 13 }}>
          <span style={{ padding: '4px 10px', background: '#dbeafe', borderRadius: 20, color: '#1d4ed8', fontWeight: 600 }}>Caso #4: Abandono ✓</span>
