/**
 * OUTPUT · Clínica Fisioterapia Demo · Pantalla 2: CRM simulado
 * Reutiliza CORE (Card, Badge, FicticioLabel). Datos ficticios de fisioterapia.
 * Sin envíos reales. Sin datos de personas reales.
 */
import { useState } from 'react';
import { Card, Badge, FicticioLabel } from '../../core/AppShell.jsx';
import { MOCK_PACIENTES_FISIO } from '../../verticals/fisioterapia/mockData.js';

const ESTADOS = ['todos', 'nuevo', 'pendiente', 'en_valoracion', 'sesion_agendada', 'perdido'];
const ESTADO_LABELS = {
  todos: 'Todos',
  nuevo: 'Nuevo lead',
  pendiente: 'Pendiente',
...
export function PhysioCrm() {
  const [filtro, setFiltro] = useState('todos');
  const [expandedId, setExpandedId] = useState(null);

  const pacientes = filtro === 'todos'
    ? MOCK_PACIENTES_FISIO
    : MOCK_PACIENTES_FISIO.filter(p => p.estado === filtro);

  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ fontSize: 20, fontWeight: 800, color: '#111827', margin: '0 0 4px' }}>📋 CRM simulado</h2>
        <p style={{ margin: 0, fontSize: 14, color: '#6b7280' }}>Pacientes fisioterapia · Datos ficticios · Sin envíos reales</p>
      </div>

      {/* Resumen */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: 20 }}>
        {[
          { label: 'Total leads',      value: MOCK_PACIENTES_FISIO.length, icon: '👥', color: '#2563eb' },
          { label: 'Nuevos',           value: MOCK_PACIENTES_FISIO.filter(p => p.estado === 'nuevo').length, icon: '🆕', color: '#7c3aed' },
          { label: 'Sesiones agendadas', value: MOCK_PACIENTES_FISIO.filter(p => p.estado === 'sesion_agendada').length, icon: '📅', color: '#059669' },
          { label: 'Pipeline est.',    value: '7.400 €', icon: '💰', color: '#d97706', ficticio: true },
        ].map(s => (
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        {ESTADOS.map(e => (
          ...
          >
            {ESTADO_LABELS[e]} {e !== 'todos' && `(${MOCK_PACIENTES_FISIO.filter(p => p.estado === e).length})`}
          </button>
        ))}
      </div>
              ...
              {pacientes.map(p => (
                <>
                  ...
                  >
                    ...
                    <td style={TD}><span style={{ fontSize: 13 }}>{p.servicio}</span></td>
                    <td style={TD}><span style={{ fontSize: 12 }}>{p.centro}</span></td>
                    ...
                    <tr key={`${p.id}-detail`} style={{ background: '#f0f9ff' }}>
                      <td colSpan={8} style={{ padding: '12px 20px' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, fontSize: 13 }}>
                          ...
                          {p.sesiones_restantes != null && (
                            <div><span style={{ color: '#6b7280' }}>Sesiones bono restantes:</span> <strong>{p.sesiones_restantes}</strong> <FicticioLabel /></div>
                          )}
                          <div><span style={{ color: '#6b7280' }}>Próxima acción:</span> <strong>{p.proxima_accion}</strong></div>
                        </div>
                        <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
                          {['Marcar contactado', 'Agendar sesión', 'Enviar información de bonos'].map(a => (
                            <button
                              key={a}
...
const TH = { padding: '10px 14px', textAlign: 'left', fontSize: 12, fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.04em', whiteSpace: 'nowrap' };
