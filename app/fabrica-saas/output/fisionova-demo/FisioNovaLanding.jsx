    ...
    <section style={{ background: C.bg, padding: '4rem 1.5rem' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.25rem' }}>
          {visibles.map(s => (
            <div key={s.id} onClick={() => setExpandido(expandido === s.id ? null : s.id)}
    ...
    <section style={{ background: C.bgWhite, padding: '4rem 1.5rem' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '2rem' }}>
          {pasos.map((p, i) => (
            <div key={p.num} style={{ textAlign: 'center', opacity: 0, animation: `fadeSlideUp 0.5s ease ${i * 0.15}s forwards` }}>
              <div style={{ width: '4rem', height: '4rem', borderRadius: '50%', background: C.bg, border: `2px solid ${C.primary}`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1rem', fontSize: '1.5rem' }}>{p.icon}</div>
              <div style={{ fontSize: '0.75rem', fontWeight: 700, color: C.primary, letterSpacing: '0.1em', marginBottom: '0.4rem' }}>PASO {p.num}</div>
              <h3 style={{ fontSize: '1rem', fontWeight: 700, color: C.text, marginBottom: '0.5rem' }}>{p.titulo}</h3>
              <p style={{ fontSize: '0.85rem', color: C.textMuted, lineHeight: 1.5 }}>{p.desc}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <section style={{ background: C.bg, padding: '4rem 1.5rem' }}>
      <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '1.25rem' }}>
          {PROFESIONALES.map(p => (
            <div key={p.id} onClick={() => setActivo(activo === p.id ? null : p.id)}
              style={{ background: '#fff', borderRadius: '1rem', padding: '1.5rem', textAlign: 'center', border: `1.5px solid ${activo === p.id ? p.color : C.border}`, cursor: 'pointer', transition: 'all 0.2s', boxShadow: activo === p.id ? `0 4px 20px ${p.color}30` : 'none' }}>
