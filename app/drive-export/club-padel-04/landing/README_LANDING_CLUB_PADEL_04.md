...
| Imagen de fondo del hero (`torcal-padel-bg.png`, con versión optimizada `torcal-padel-bg.webp`) + vista previa social (`og-image.jpg`) + iconografía | `../assets/images/` |
...
- [x] **Imagen de fondo del hero (`torcal-padel-bg.png`) — documentada correctamente y overlay mejorado (2026-07, mejora 4B):** su origen no está confirmado como fotografía real (pertenece al mismo lote de generación que archivos `ChatGPT Image *.png` de la carpeta de descargas del usuario). Tratada en toda la documentación como **imagen visual de marca / recurso visual hero**, nunca como "foto real del club" (ver Brand System §2). Se evaluaron candidatas locales para sustituirla y ninguna resultó una mejora clara y segura, así que se mantuvo la misma imagen y se aclaró el overlay para que el fondo se vea más sin perder legibilidad.
- [ ] **Fotos reales del club (galería):** hallazgo relevante (2026-07): en la carpeta de descargas del usuario existen 5 imágenes distintas entre sí que podrían resolver la galería pendiente — no se han incorporado en esta tarea (fuera de alcance), su origen tampoco está confirmado como real.
...
- [x] **Rendimiento del hero (2026-07, pulido SEO/performance):** generado `torcal-padel-bg.webp` (calidad 88) a partir del mismo `torcal-padel-bg.png`, sin sustituir la imagen ni cambiar el encuadre — ~210 KB frente a ~2,17 MB del PNG, servido vía `image-set()` en CSS con el PNG como fallback.
- [x] **`og:image` conectado (2026-07):** generado `og-image.jpg` (1200×630, recorte del mismo `torcal-padel-bg.png`, sin foto nueva) y añadidas las etiquetas Open Graph/Twitter Card en `index.html`. Rutas relativas (sin dominio real todavía) — validar en un validador externo antes de publicar.
