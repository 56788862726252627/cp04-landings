...
- [x] **Imagen de fondo del hero (`torcal-padel-bg.png`) — documentada correctamente:** su origen no está confirmado como fotografía real (sin metadatos EXIF/cámara al inspeccionarla). Se corrigió toda la documentación (`BRAND_SYSTEM_CLUB_PADEL_04.md`, `FIGMA_LANDING_SPEC_CLUB_PADEL_04.md`) para tratarla como **imagen visual de marca / recurso visual hero**, nunca como "foto real del club". Pendiente, si se quiere fotografía auténtica: sustituir por una foto real verificada del club antes de un despliegue con esa promesa.
