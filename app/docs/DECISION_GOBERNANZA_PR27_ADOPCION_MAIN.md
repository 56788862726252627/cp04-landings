<!--
COPIA DOCUMENTAL DE REFERENCIA — NO ES LA FUENTE CANÓNICA.

- Fuente original: /root/cp04-landings/.claude/worktrees/release-governance-decision-doc/app/docs/club-padel-04/comunidad/DECISION_ADOPCION_MAIN_LINEA_AVANZADA_CLUB_PADEL_04.md
  (worktree "release-governance-decision-doc", contenido de PR #27 en GitHub).
- Fecha de copia: 2026-07-18.
- Motivo: dejar visible en app/docs/ (Terminal 6, rama frontend/audit-fixes-20260709)
  la cadena de gobernanza PR #24 → PR #26 → PR #27 → PR #36, para evitar confusión
  entre terminales/sesiones sobre por qué PR #36 permanece en draft.
- Este archivo NO autoriza merge, deploy ni cambio de estado de ningún PR.
  Solo PR #27 (GitHub) es la fuente autoritativa de esta decisión.
- Estado vigente al momento de la copia: PR #24/#26/#27/#36 siguen OPEN/draft.
  PR #36 está técnicamente listo (148/148 tests Worker, build OK) pero debe
  permanecer en draft hasta que se resuelva esta cadena de gobernanza.
-->
...
## Adenda de esta copia (2026-07-18) — eslabón PR #36

El documento fuente original (PR #27) es anterior a la apertura de PR #36 y por tanto no lo menciona. Para que esta copia refleje la cadena de gobernanza vigente confirmada en auditorías posteriores (2026-07-18):

- **PR #36** ("Make 50/50": caché de disponibilidad, modo degradado Airtable 429, idempotencia de reservas, observabilidad — Pasos 06A-06E) tiene como base `release/staging-club-padel-04-2026-07-15` (la misma rama de PR #26), no `main`.
- Está técnicamente listo (148/148 tests Worker, 129/129 tests frontend, build OK, `mergeable: MERGEABLE`, `mergeStateStatus: CLEAN`) pero permanece **en draft por la misma cadena de gobernanza** descrita arriba: depende de que PR #26 se mergee, que a su vez depende de resolver PR #24.
- Cadena completa vigente: **PR #24 (legal/menores) → PR #26 (staging/adopción de `main`) → PR #27 (esta decisión) → PR #36 (Make 50/50)**.
- Ninguna condición nueva sustituye las condiciones obligatorias ya listadas arriba; PR #36 simplemente hereda el mismo bloqueo.
